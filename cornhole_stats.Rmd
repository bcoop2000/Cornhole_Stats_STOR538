...
by_round_BC <- Cornhole%>%
  group_by(Match, Round, Thrower)%>%
  summarise(sum(Strategic_Block_Created))
by_round_BC
...
by_game_NBS1 <- Cornhole%>%
  group_by(Match)%>%
  mutate(total_on_board = sum(On_Board))

by_game_NBS <- by_game_NBS1%>%
  group_by(Match, Thrower)%>%
  summarise(flat_NBS = sum(Opponent_Other_Bags_Knocked_Off) - sum(Own_Other_Bags_Knocked_Off) + 2*sum(Own_Other_Bags_Dragged_In) - 2*sum(Opponent_Bags_Dragged_In),
            norm_NBS = (sum(Opponent_Other_Bags_Knocked_Off) - sum(Own_Other_Bags_Knocked_Off) + 2*sum(Own_Other_Bags_Dragged_In) - 2*sum(Opponent_Bags_Dragged_In))/total_on_board)
by_game_NBS
```
