...
```{r}
Throw_breakdown <- data%>%
  group_by(Throw_Number)%>%
  summarise(total = n(),
            in_hole_pct = round(sum(InHole.)/total*100,1),
            on_board_pct = round(sum(OnBoard.)/total*100,1),
            missed_board_pct = round(sum(MissedBoard.)/total*100,1),
            ...
            Hole_Blocked_After_pct = round(sum(Hole_Blocked.)/total*100,1))
...
```{r}
blocked_by_throw_plot <- plot(Throw_breakdown$Throw_Number, Throw_breakdown$Hole_Blocked_Before_Throw_pct, xlab = "Throw", ylab = "Hole Blocked")
